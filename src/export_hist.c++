...
void init_channels(void) {
    ...
    for(int iChannel=0; iChannel<nChannels; ++iChannel)
        tuple_vars[4+iChannel]="s"+channel_name[iChannel];
    for(int i=0; i<4+nChannels; ++i) {
...
int nBins=30;
string out_prefix="";
dbl_type Q2, minQ2, maxQ2, Y, pTpsi, minPT2, maxPT2, W2, minW2, maxW2;
int nEv=0;
...
void fill_hst() {
    for(int iChannel=0; iChannel<nChannels; ++iChannel)
        dSigma[iChannel]=tuple_vals[4+iChannel]/nEv;
    for(int iChannel=0; iChannel<nChannels; ++iChannel) {

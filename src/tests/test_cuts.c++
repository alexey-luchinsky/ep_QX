...
TEST_CASE("Q2Ycut_flat_sum") {
    ...
    dbl_type m1 = random_generator->rand(0, ecm_/ 2), 
            m2 = random_generator->rand(0, ecm_ / 2);
    dbl_type minQ2=0, midQ2=0.5*s_, maxQ2=s_;
    dbl_type minY=0.1, maxY=0.9;
    RamboEP ramEP_all(ecm, random_generator, m1, m2); 
    ramEP_all.set_minmaxQ2(minQ2,maxQ2); ramEP_all.set_minmaxY(minY,maxY);
    RamboEP ramEP_left(ecm, random_generator, m1, m2); 
    ramEP_left.set_minmaxQ2(minQ2, midQ2); ramEP_left.set_minmaxY(minY,maxY);
    RamboEP ramEP_right(ecm, random_generator, m1, m2); 
    ramEP_right.set_minmaxQ2(midQ2, maxQ2); ramEP_right.set_minmaxY(minY,maxY);
    dbl_type sum_all = 0, sum_left=0, sum_right=0;
    for (int iEv = 0; iEv < nEv; ++iEv) {
        ramEP_all.next(x);  ramEP_left.next(x); ramEP_right.next(x);
        sum_all += ramEP_all.wt/nEv;
    ...
    dbl_type sum_sum=sum_left+sum_right;
    cout<<"[Q2Ycut_flat_sum] sum_all="<<sum_all<<" sum_sum="<<sum_sum<<" sum_all/sum_sum="<<sum_all/sum_sum<<endl;
    cout<<"[Q2Ycut_flat_sum] ramEP_all.nFault="<<ramEP_all.nFault<<
            " ramEP_left.nFault="<<ramEP_left.nFault<<
            " ramEP_right.nFault="<<ramEP_right.nFault<<endl;
    REQUIRE(sum_all/sum_sum==Approx(1).epsilon(0.01));
}
